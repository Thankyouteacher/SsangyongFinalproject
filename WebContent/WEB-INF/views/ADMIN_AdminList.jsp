...
<title>ADMIN_AccountUsertList.jsp</title>
<!-- Bootstrap core CSS -->
...
<script type="text/javascript">
	$(document).ready(function()
	{
		...
		$(".AdminAccountDelete").click(function()
		{
			//alert("관리자 계정을 부여하시겠습니까?");
			var message = confirm("관리자 계정을 삭제하시겠습니까?");
			if (message == true)
			{
				location.href="admindelete.action?adminId=" + $(this).val();
			}
			else if (message == false)
			{
				location.href="adminlist.action"	;		
			}
			else
			{
				location.href="adminlist.action"	;		
		...
		<div class="row">
			...
			<div class="col-lg-9">
				<div class="card card-outline-secondary my-4"
					...
					<div class="card-header">관리자 관리</div>
					<div class="card-body">
				
				<!--  검색 기능 -->
				<form name="form1"  method="get" action="Admin_searchname.action">
						...
						<table class="table table-striped">
							<thead>
								<tr>
									...
									<th style="text-align: center">관리자아이디</th>
									<th style="text-align: center">관리자부여날짜</th>
									<th style="text-align: center">관리자 유형</th>
									<th style="text-align: center">관리자 계정 박탈</th>
								</tr>
							<tbody>
								<c:forEach var="admin" items="${admin_list }">
									<tr>
										<th style="text-align: center">${admin.uname }</th>
										<th style="text-align: center">${admin.nickName }</th>
										<th style="text-align: center">${admin.adminId}</th>
										<th style="text-align: center">${admin.adminDate}</th>
										
											<c:choose>
												<c:when test="${admin.gadminId==1}"> <th>총관리자</th></c:when>
												<c:otherwise> <th>일반관리자</th></c:otherwise>
											</c:choose>
										<th><button type="button" class="btn AdminAccountDelete" value="${admin.adminId }" style=" background-color: #E53A40 ;color : white; height: 5%">관리자박탈</button></th>
									</tr>
								</c:forEach>
							</tbody>
						</table>
