...
<script type="text/javascript">
	...
	$(document).ready(function()
	{
		...
		$('.searchBtn').click(function()
		{
			...
			$(location).attr('href', 'searchboard.action?bid=2&category=' + category + "&content=" + content);
		});
		
		$('#write').click(function()
		{
			$(location).attr('href', 'boardwrite.action?bid=2');
		});
		...
		<div class="col-md-9 float-right" style="width: 850px;">
			<table class="table table-hover table-striped" style="text-align: center;">
				...
				<tbody id="tbody" style="text-align: right;">
					...
					<tr onclick="location.href='boardread.action?bid=${infoList.bid }&bpid=${infoList.bpid }&usId=${usId }'">
						<th style="text-align: center;">${infoList.bpid }</th>
						<th style="text-align: center;">${infoList.postTitle }</th>
						<th style="text-align: center;">${infoList.nickName }</th>
						<th style="text-align: center;">${infoList.createDate }</th>
						<th style="text-align: center;">${infoList.click }</th>
						<th style="text-align: center;">${infoList.likeCount }</th>
					</tr>				
					</c:forEach>
				</tbody>
			</table>
